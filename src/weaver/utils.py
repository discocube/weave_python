"""
This module contains classes and functions related to cycle merging.

class Weft: Responsible for merging other cycles into it and incrementing
    the elevation at which the cycles are joined / merged and getting the edges
    and adjacent edges of weft.data.
    __init__(self, data: Tour, n: int) -> None:
        Set data and initialize [order, z_abs_max, zc_abs_sum] using n.
    @property edges(self) -> Edges:
        Tuple windows from data and filter according to conditions to simplify.
    @staticmethod get_eadj(edge: Edge) -> Edge:
        Get one parallel edge from given edge based on if joined or not.
    join(self, other: Tour) -> None:
        Join self with other after it has been aligned.

class LoomThread:
    Responsible for getting the edges and adjacent edges of the threads left in the loom.
    @staticmethod get_edges(data: Tour, joined: bool = True) -> Edges:
        Get edges of thread and filter based on joined.
    @staticmethod get_eadjs(edges: Edges) -> Edges:
        Get adjacent edges of edges and filter to shorten data.
"""
...
class Weaver:
    """
    Weaver is responsible for merging other cycles into it and
    incrementing the elevation at which the cycles are joined/merged.
    ...
    def __init__(self, data: Tour, radius: int, order: int) -> None:
        ...
        # Elements are merged at different axes if joined or not, hence the need for a switch.
        self.joined: bool = False
        # The maximum absolute scalar value for z.
        self.z_abs_max: int = radius - 4
        # Initial value to determine elevation of the stitch (merge)
        # and incremented to elevate position.
    ...
    def get_eadj(edge: Edge) -> Edge:
        ...
        Returns:
            Edge: Adjacent edge of edge. If edge lies along the x axis
                return the parallel edge one positive step along the y-axis.
            Otherwise return the parallel edge one positive step along the x-axis.
        """
        return {
            # if edge lies along x-axis: the adjacent parallel edge is one step along the y-axis.
            (
                (edge[0][0], edge[0][1] + 2, edge[0][2]),
                (edge[1][0], edge[1][1] + 2, edge[1][2]),
            )
            if edge[0][0] != edge[1][0]
            # if edge lies along y- or z-axis: the parallel/adjacent edge is one tep in the x-axis.
            else ((edge[0][0] + 2, *edge[0][1:]), (edge[1][0] + 2, *edge[1][1:]))
        }
...
class LoomThread:
    ...
    def get_edges(data: Tour, joined: bool = True) -> Edges:
        ...
        return {
            ...
            # check if it matches this pattern if joined:
            # ((3, 1, _), (_, 1, _)) else ((1, 3, _), (_, 3, _))
