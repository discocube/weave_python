...
- spin_yarn(radius: int) -> Spool:
    Spin a 2D zig-zagging outwards turning Hamiltonian path from the innermost vertex [-1, 1] or [1, 1] to [radius, 1].
...
- merge_cycles(loom: Loom, n: int) -> Solution:
    Merge cycles by finding bridge edges between two loops, aligning them, and joining them.
...
from src.weaver.types import *

def spin_yarn(radius: int) -> Spool:
    """Spin a 2d zig-zagging outwards turning Hamiltonian path from the innermost vert [-1, 1] or [1, 1] to [radius, 1]

    Args:
        r (int): Radius of Manhattan Ball with an edge-length of 2.

    Returns:
        Spool: A Hamiltonian Chain of 2d vectors where the first item is either [-1, 1] or [1, 1] and the last item is [radius, 1]
    """
    ...
    (start_x, dpv) = (
        (1, zigzags) if not ((radius + 1) // 2) % 2 else (-1, zigzags[2:] + zigzags[:2])
    )
    # Return path which is an accumulation starting with the start vector and using collected zigzag vectors as an iterator getting the next step by adding a zigzag vector.
    return {
        # The original spun is blue
        3: (
            spun := [
                *accumulate(
                    (  # use modulo to cycle through zigzags: idx % 4 for an xy zigzag set and (idx + i) % 2 alternates between x and y.
                        dpv[idx % 4][(idx + isize) % 2]
                        for idx, size in enumerate(
                            # start from 1 to the radius, odd steps: if r = 5: (1, 3, 5)
                            # repeat three times if it is the radius else twice: [[1, 1], [3, 3], [5, 5, 5]] then flatten
                            [
                                item
                                for nested in [
                                    [x] * (2 if x != radius else 3)
                    ...
                    func=lambda x, ac: (x[0] + ac[0], x[1] + ac[1]),
                    # The initial value is either [-1, 1] or [1, 1]
                    initial=(start_x, 1),
                )
            ]
        ),
        ...
        1: [
            (x, y)
            for x, y in np.add(np.dot(np.array(spun), [[-1, 0], [0, -1]]), [0, 2])
        ],
    }


def pin_thread_ends(loom: Loom, z: int) -> Pins:
    ...
    for thread in loom:
        ...
        thread.appendleft(left := (*thread[0][:2], z))
        # append-right and assign appended to right
        thread += [right := (*thread[-1][:2], z)]
...
def chop(tour: Tour, pins: Pins) -> Subtours:
    ...
    for i, j in reversed(list(enumerate(x for x, t in enumerate(tour) if t in pins))):
        ...
        if not i and tour[0] not in pins:
            subtours += [tour[: j + 1][::-1]]
            # Does not always result in a tour so we need filter to avoid going through the whole list to check for empties.
...
def extend_threads(loom: Loom, chopped: Subtours) -> None:
    """Extend each thread of the loom with the matching chopped segment.
    ...
    used = set()
    for thread in loom:
        for idx, ch in enumerate(chopped):
            if idx not in used:
                if thread[0] == ch[0]:
                    thread.extendleft(ch[1:])
                    used |= {idx}
                elif thread[-1] == ch[0]:
                    thread += ch[1:]
                    used |= {idx}
    loom += [deque(ch) for idx, ch in enumerate(chopped) if idx not in used]
...
def merge_cycles(loom: Loom, radius: int, order: int) -> Solution:
    """Merge cycles by finding bridge edges between two loops, aligning them to those edges and then joining
    ...
    Returns:
        Solution: Solution as a list of vectors resulting from the merging of subcycles into one cycle.
    """
    ...
    for thread in loom:
        ...
        # align weft to weft bridge so that the lhs and rhs of weft.data matches those of the weft bridge.
        Weaver.align_to(weaver.data, *weft_bridge)
        # align current thread to a reversed bridge edge resulting from the intersection of thread edges and adjacent edge of weft_bridge.
        Weaver.align_to(
