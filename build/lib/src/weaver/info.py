"""
This module contains helper functions to get information for n. This was once a graph class with adjacency lists etc.,
but has now been reduced to these functions to reduce memory use. So technically, this is a graph info class.

The module provides the following functions:

- get_z_color_size(n: int) -> ZInfo: Get z-scalar value, color, and size for each negative level from -radius to -1.
- get_order_from_radius(radius: int) -> int: Get the order from the given radius of a Manhattan ball.
- get_order_from_n(n: int) -> int: Get the number of vertices of an n-layer of cubes with the core included.
- get_radius_from_n(n: int) -> int: Get the radius for a given number of layers around a central cube.
- get_n_from_radius(radius: int) -> int: Get n from radius where the edge-length is 2 units.
"""
...
def get_z_color_size(n: int) -> ZInfo:
    ...
    return zip(
        ...
        [n for nest in repeat(((1, 3) if not n % 2 else (3, 1)), n) for n in nest],
        # Order for each level:
        map(lambda f: 2 * f * (f + 1), range(1, n + 1)),
    )
...
def get_order_from_n(n: int) -> int:
    ...
    """
    return (4 * (n + 2) * (n + 1) * n) // 3


def get_radius_from_n(n: int) -> int:
